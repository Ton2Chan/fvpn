...
# ------------------------------------------------------------------------------
# should_check_update
# 機能: サーバーリストの自動更新チェックを実行すべきか判定する
# 戻り値: 0 (チェック実行すべき), 1 (スキップすべき)
# ------------------------------------------------------------------------------
should_check_update() {
    # 1. 初回起動/未更新時（LAST_UPDATE_TS が 0 または未設定）は必須実行
    ...
    # 2. UPDATE_CHECK_INTERVAL (完全封印期間) の判定
...
get_download_dir() {
    ...
    # 1. xdg-user-dir コマンドが存在すれば実行して取得
    if command -v xdg-user-dir >/dev/null 2>&1; then
        dl_dir=$(xdg-user-dir DOWNLOAD 2>/dev/null)
    fi

    # 2. コマンドで取れず、user-dirs.dirs ファイルが存在すれば解析
    ...
    # 3. どちらでも取れない場合のフォールバック ($HOME/Downloads)
        ...
        while IFS='=' read -r key val || [ -n "$key" ]; do
            ...
            case "$key" in
                ...
                CONNECT_MODE)
                    ...
                    ;;
                PROTOCOL_MODE)
                    # -1: TCP, 0: MIX, 1: UDP (正の数/0/負の数の判定パターン)
...
resolve_work_line() {
    ...
    # 15文字固定長 (右パディングスペース埋め)
    printf "%s %-15s\n" "$key" "$ip"
}
export -f resolve_work_line

# ==============================================================================
# build_master_servers (本体関数: 16並列IP引き制御を内包する2関数構成)
# ==============================================================================
build_master_servers() {
    ...
    # 2. 全 .ovpn から 「u/ファイル名 ドメイン名 ポート番号」 の作業リストを作成 (CR除去付き)
    local work_src=""
    if [ -d "$udp_dir" ]; then
    ...
    # 3. 16並列 IP 引きの実行と連想配列 (IP_MAP / PORT_MAP) への格納
    declare -A IP_MAP
    declare -A PORT_MAP
    ...
    # 4. 単一ループで固定長バッファを一括生成
    local new_buf=""
    local udp_cnt=0
    local tcp_cnt=0
    local fname_field_len=$(( RECORD_BYTE_SIZE - OFFSET_PROTO - 1 )) # 94バイト
...
# 機能: マスターバッファのロードとUDP/TCP/合計サーバー件数のカウント
# ------------------------------------------------------------------------------
load_master_servers_and_count() {
...
# --------------------------------------------------------------------------
    # 2.5 起動時セルフヒーリング (生IPかつ非VPN状態なら残存ルールを完全クリーンアップ)
    # --------------------------------------------------------------------------
    if is_network_online && ! ip link show tun0 >/dev/null 2>&1; then
        phy_disconnect >/dev/null 2>&1
    fi

    # 3. IS_FIRST_BOOT を活用したカウント＆ロードの分岐
    load_master_servers_and_count

    # 4. 設定ファイル (.ini) の読み込み
    load_all_settings

    # 5. 起動時アップデート処理 (1パックアップデート関数の呼び出し)
    # IS_FIRST_BOOT=0 かつ HAS_UPDATE_ZIP=0 の場合のみupdate_servers "skip"
    ...
    # 6. アップデート後のバッファ再ロード ＆ アクティブバッファ構築
    if [ -f "${FILE_MASTER_SERVERS}" ]; then
        MASTER_SERVERS_BUFFER="$(cat "${FILE_MASTER_SERVERS}" 2>/dev/null)"$'\n'
    fi
