...
# 1. Logical Layer / 論理層
export CONNECT_MODE=1     # 1: FIXED, 2: SEQUENTIAL, 3: RANDOM / 1: 固定(FIX), 2: 順番(SEQ), 3: ランダム(RAND)
export PROTOCOL_MODE=0    # 0: MIX (UDP+TCP), 1: UDP Only, -1: TCP Only / 0: MIX, 1: UDP専用, -1: TCP専用
export FILTER_LEVEL=1     # 1: Show All, 0-4: Rating Filter / 1: 全表示, 0〜4: 評価フィルター

# Language Switch Setting (FVPN_LANG -> System LANG fallback if unset)
# 言語切替設定 (FVPN_LANG -> 未指定時はシステムLANGから自動判別)
...
export TIMEOUT_VPN_CONNECT="${TIMEOUT_VPN_CONNECT:-30}" # VPN Connection Timeout in seconds (Default: 30s) / VPN接続タイムアウト秒数 (デフォルト30秒)
export CURRENT_TIMEOUT="${TIMEOUT_MANUAL_UPDATE}"
export UPDATE_CHECK_INTERVAL=7     # Cooldown period after successful update (7 days) / 更新成功後の完全封印期間 (7日)
export UPDATE_RETRY_INTERVAL=1     # Skip interval on failure/no update (1 day) / チェック失敗/未更新時のスキップ期間 (1日)

# ------------------------------------------------------------------------------
# Data Structure Constants (128-byte Fixed Length Buffer Definition)
# データ構造仕様定数 (128バイト固定長バッファ定義)
# ------------------------------------------------------------------------------
export RECORD_BYTE_SIZE=128      # Fixed byte length per record / 1レコードあたりの固定長バイト数

export MARK_NORMAL="_"           # Normal server mark / 通常サーバーマーク
export MARK_ADDED="+"            # Newly added server mark / 新規追加サーバーマーク

export OFFSET_ADDED_MARK=0       # Added mark position ("_ " or "+ ") / 新規追加マーク位置 ("_ " または "+ ")
export LENGTH_ADDED_MARK=1

export OFFSET_INDEX=2            # Server index number (e.g. "   1") / サーバー通し番号 (例: "   1")
...
export LENGTH_PORT=5             # Port number length (Fixed 5 bytes) / ポート番号長 (固定5バイト)

export OFFSET_PROTO=33           # Protocol type ("u" or "t") / プロトコル種別 ("u" または "t")
...
# Internal Control Paths for Physical Layer / 物理層内部ステータス・制御用ファイルパス (統合)
...
# DNS Protection & Backup Paths / DNS保護・退避用ファイルパス (追加)
...
# 3. On-Memory Buffers & State Management Variables / オンメモリバッファ & 状態管理変数
export MASTER_SERVERS_BUFFER=""
export ACTIVE_SERVERS_BUFFER=""

# Target Server Management Variables / 注目サーバー管理変数 (名前ベース追跡対応)
export TARGET_INDEX=-1           # Target server index (-1: Unselected) / 注目サーバーインデックス (-1: 未選択, 0〜: 有効インデックス)
export TARGET_SERVER=""          # Target server identifier name / 注目サーバー識別子名 (未選択は "")

# Boot State Flags / 起動状態判定用フラグ (初期化プロセス用スナップショット)
export IS_FIRST_BOOT=0      # 1: First Boot / 1: 初回起動 (.master_servers_fixed.txt が存在しない)
export HAS_UPDATE_ZIP=0     # 1: zip file exists in Downloads / 1: ダウンロードフォルダに fastestvpn_ovpn.zip が存在

# Master Server Counts / マスターサーバー件数 (マスターバッファ走査時にセット)
export FVPN_UDP_COUNT=0
export FVPN_TCP_COUNT=0
export FVPN_TOTAL_COUNT=0

# Active (Filtered) Server Counts / フィルター通過(アクティブ)サーバー件数
...
# 4. Environment Assurance / 物理環境保証
