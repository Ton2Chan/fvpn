...
# ------------------------------------------------------------------------------
# should_check_update
# Function: Determine whether automatic server list update check should run
# Return: 0 (Should check), 1 (Should skip)
# ------------------------------------------------------------------------------
should_check_update() {
    # 1. First boot / Unupdated state (LAST_UPDATE_TS is 0 or unset): Must run
    ...
    # 2. UPDATE_CHECK_INTERVAL (Full lock period) evaluation
    ...
    if [ "$check_interval_days" -gt 0 ]; then
        ...
        if [ "$diff_up" -lt "$check_interval_sec" ]; then
            return 1 # Skip during lock period
        fi
    fi

    # 3. UPDATE_RETRY_INTERVAL (Check retry interval) evaluation
    local retry_interval_days="${UPDATE_RETRY_INTERVAL:-1}"
    if [ "$retry_interval_days" -eq 0 ]; then
        return 0 # Execute check on every boot when set to 0
    fi

    local retry_interval_sec=$(( retry_interval_days * 86400 ))
    local diff_chk=$(( now_ts - last_chk ))
    if [ "$diff_chk" -ge "$retry_interval_sec" ]; then
        return 0 # Execute due to elapsed check interval
    fi

    return 1 # Skip as interval has not elapsed
}

# ------------------------------------------------------------------------------
# Get Download Directory Path (XDG Standard Compliant)
# ------------------------------------------------------------------------------
get_download_dir() {
    local dl_dir=""

    # 1. Execute xdg-user-dir if command exists
    if command -v xdg-user-dir >/dev/null 2>&1; then
        dl_dir=$(xdg-user-dir DOWNLOAD 2>/dev/null)
    fi

    # 2. Parse user-dirs.dirs file if command failed and file exists
    ...
    # 3. Fallback path ($HOME/Downloads) if both methods fail
...
apply_work_zip_package() {
    ...
    local stage_dir="${data_dir}/_update_staging"

    [ ! -f "$work_zip" ] && return 1

    local udp_dir="${FVPN_UDP:-${base_dir}/udp_files}"
    local tcp_dir="${FVPN_TCP:-${base_dir}/tcp_files}"
...
load_all_settings() {
    ...
    if [ -f "$ini_file" ]; then
        while IFS='=' read -r key val || [ -n "$key" ]; do
            ...
            case "$key" in
                TARGET_SERVER)
                    # Adopt target server identifier as-is
                    TARGET_SERVER="$val"
                    ;;
                CONNECT_MODE)
                    # 1: FIX, 2: SEQ, 3: RAND
                ...
                UPDATE_CHECK_INTERVAL | UPDATE_RETRY_INTERVAL)
                    # Allow non-negative integers (days)
                    if [[ "$val" =~ ^[0-9]+$ ]]; then
                        eval "$key=\"$val\""
                    fi
                    ;;
                LAST_CHECK_TS | LAST_UPDATE_TS)
                    # Epoch timestamp (non-negative integer)
                    if [[ "$val" =~ ^[0-9]+$ ]]; then
                        eval "$key=\"$val\""
                    fi
                    ;;
                LAST_CHECK_DATE | LAST_UPDATE_DATE)
                    # Date string (Adopt if non-empty)
                    if [ -n "$val" ]; then
                        eval "$key=\"$val\""
                    fi
                    ;;
...
# ------------------------------------------------------------------------------
# resolve_work_line (Required worker function invoked via xargs -P 16)
# ------------------------------------------------------------------------------
resolve_work_line() {
    local key="$1"
    local domain="$2"
    ...
    # Fixed-length 15 chars (Right-padded with spaces)
    printf "%s %-15s\n" "$key" "$ip"
}
export -f resolve_work_line

# ==============================================================================
# build_master_servers (Core function: 2-function structure with 16 parallel DNS lookups)
# ==============================================================================
build_master_servers() {
    local default_mark="${1:-$MARK_NORMAL}"
    local udp_dir="${FVPN_UDP_DIR:-${FVPN_HOME}/udp_files}"
    local tcp_dir="${FVPN_TCP_DIR:-${FVPN_HOME}/tcp_files}"

    # 1. Preserve existing ratings map from buffer
    declare -A old_map
    ...
    # 2. Extract "u/filename domain_name port_number" worklist from all .ovpn files (with CR removal)
    local work_src=""
    if [ -d "$udp_dir" ]; then
    ...
    # 3. Execute 16-parallel DNS lookups and store results in associative arrays (IP_MAP / PORT_MAP)
    declare -A IP_MAP
    declare -A PORT_MAP
    ...
    # 4. Construct fixed-length buffer in a single pass loop
    local new_buf=""
    local udp_cnt=0
    local tcp_cnt=0
    local fname_field_len=$(( RECORD_BYTE_SIZE - OFFSET_PROTO - 1 )) # 94 bytes
...
# Function: Load master buffer and count UDP/TCP/Total server quantities
# ------------------------------------------------------------------------------
load_master_servers_and_count() {
...
init_boot_process() {
    # 1. Prepare physical directories
    mkdir -p "${FVPN_DATA}" "${FVPN_LOG}"

    # 2. Flag determination (Boot-time snapshot)
    ...
    # --------------------------------------------------------------------------
    # 2.5 Boot Self-Healing (Clean up residual rules if network is raw IP & non-VPN)
    # --------------------------------------------------------------------------
    if is_network_online && ! ip link show tun0 >/dev/null 2>&1; then
        phy_disconnect >/dev/null 2>&1
    fi

    # 3. Count & Load branching utilizing IS_FIRST_BOOT
    load_master_servers_and_count

    # 4. Load configuration file (.ini)
    load_all_settings

    # 5. Boot update process (Invoke single-pack update function)
    # Execute update_servers "skip" only when IS_FIRST_BOOT=0 and HAS_UPDATE_ZIP=0
    ...
    # 6. Reload buffer post-update & Build active buffer
    if [ -f "${FILE_MASTER_SERVERS}" ]; then
        MASTER_SERVERS_BUFFER="$(cat "${FILE_MASTER_SERVERS}" 2>/dev/null)"$'\n'
    fi
